 ...
 *
 * Example: ts-node scripts/create-fhevm-example.ts confidential-pension ./my-pension-example
 */
...
const EXAMPLES_MAP: Record<string, ExampleConfig> = {
  'confidential-pension': {
    ...
    description: 'A comprehensive FHEVM pension system demonstrating encrypted financial operations including contributions, returns calculation, and secure withdrawals',
  },
  'fhe-arithmetic': {
    contract: 'contracts/basic/FHEArithmetic.sol',
    test: 'test/basic/FHEArithmetic.ts',
    description: 'Demonstrates FHE arithmetic operations: add, subtract, multiply, and divide on encrypted values',
  },
  'encrypt-single-value': {
    contract: 'contracts/basic/EncryptSingleValue.sol',
    test: 'test/basic/EncryptSingleValue.ts',
    description: 'Shows how to encrypt and store a single encrypted value with proper permission management',
  },
  'encrypt-multiple-values': {
    contract: 'contracts/basic/EncryptMultipleValues.sol',
    test: 'test/basic/EncryptMultipleValues.ts',
    description: 'Demonstrates handling multiple encrypted values in a single contract',
  },
  'user-decrypt-single-value': {
    contract: 'contracts/basic/UserDecryptSingleValue.sol',
    test: 'test/basic/UserDecryptSingleValue.ts',
    description: 'Shows user decryption pattern where users decrypt values off-chain with their private key',
  },
  'public-decrypt-single-value': {
    contract: 'contracts/basic/PublicDecryptSingleValue.sol',
    test: 'test/basic/PublicDecryptSingleValue.ts',
    description: 'Demonstrates public decryption that reveals encrypted values on-chain for all to see',
  },
  'access-control': {
    contract: 'contracts/basic/AccessControl.sol',
    test: 'test/basic/AccessControl.ts',
    description: 'Shows FHE.allow and FHE.allowThis permission management for encrypted values',
  },
};
...
function main(): void {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    ...
    log('  ts-node scripts/create-fhevm-example.ts confidential-pension ./my-pension-example\n');
    process.exit(0);
  }
